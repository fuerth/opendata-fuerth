...
function showWordCloud(id, names) {
	...
	function fillColor(d ,i) {
		return `rgba(0, 0, 0, ${1.2-(d.text.length/maxLength)})`;
	}
	...
	var layout = cloud()
		...
		.words(
			Array.from(words.entries()).map(function(data) {
				return { text: data[0], size: 15+(data[1]*10) };
			})
		)
		.padding(1)
		.rotate(function() {
...
function drawSeatsChart(chartData) {
	var data = {
		labels: chartData.map(d => d.name),
		datasets: [
				{
						data: chartData.map(d => d.seats),
						backgroundColor: chartData.map(d => getColorForParty(d.name)),
						hoverBackgroundColor: chartData.map(d => getColorForParty(d.name))
				}]
	};

	new Chart(document.getElementById("seats"), {
		type: 'doughnut',
		data: data,
		options: {
			rotation: 1 * Math.PI,
