...
function filterInStadtrat(data) {
	return data.filter(r => (r.voted === 'TRUE'));
}
...
async function getSeatsByParty() {
	let entries = [];
	for (let sheet of doc.sheetsByIndex) {
		if (sheet == doc.sheetsByIndex[0]) continue;
...
async function getAllByFieldname(field) {
	let entries = [];
	for (let sheet of doc.sheetsByIndex) {
		if (sheet == doc.sheetsByIndex[0]) continue;
		let rows = await sheet.getRows();
...
(async () => {
	...
	const jobs = await getAllByFieldname('job');
